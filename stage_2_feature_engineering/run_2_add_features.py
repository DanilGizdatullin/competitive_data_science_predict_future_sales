...
full_df = pd.read_csv(os.path.join(PATH_TO_DATA, 'full_dataframe.csv'))

full_df['date_block_num'] = full_df['date_block_num'].astype(np.int8)
full_df['shop_id'] = full_df['shop_id'].astype(np.int8)
full_df['item_id'] = full_df['item_id'].astype(np.int16)

full_df = add_shops_items_category_features(full_df)
full_df = add_lag_feature(full_df, [1, 2, 3], 'item_cnt_month')
full_df = add_mean_encoded_features(full_df)
full_df = add_trend_features(full_df, train)
full_df = add_month_and_days_in_month(full_df)

full_df.to_csv(os.path.join(PATH_TO_DATA, 'full_df_with_features.csv'), index=False)
